export function normalizeReport(parsed, spec) {
...
export function buildComparison(items, reportName, generatedAt) {
  return {
    schema_version: 1,
    ...
    stores: items,
    resources: resourceValues(items),
    scenarios: unionRows(items, "scenarios"),
    endpoints: unionRows(items, "endpoints"),
  };
}

export function markdownComparison(report) {
  const lines = [];
  ...
  addResourceTables(lines, report.stores);
  ...
  lines.push("- Resource gauges use average, nearest-rank p95, and peak; cumulative Docker counters use reset-aware deltas.");
  lines.push("- JSON output contains the same store, resource, scenario, and endpoint tables for CI trend storage.");
  lines.push("");
  return `${lines.join("\n")}\n`;
}

function resourceValues(items) {
  const values = {};
  for (const item of items) {
    values[item.store] = item.resources || null;
  }
  return values;
}
...
function addResourceTables(lines, stores) {
  lines.push("");
  lines.push("## RegiMux Container Resources");
  lines.push("");
  lines.push("| metadata store | samples | interval |");
  lines.push("| --- | ---: | ---: |");
  for (const item of stores) {
    const resources = item.resources;
    lines.push(`| ${item.store} | ${resources ? formatNumber(resources.sample_count, 0) : "n/a"} | ${resources ? `${formatNumber(resources.sampling_interval_ms, 0)} ms` : "n/a"} |`);
  }

  lines.push("");
  lines.push("### Gauges");
  lines.push("");
  lines.push("| metadata store | metric | avg | p95 | peak |");
  lines.push("| --- | --- | ---: | ---: | ---: |");
  const gauges = [
    ["CPU", "cpu_percent", formatResourcePercent],
    ["memory usage", "memory_bytes", formatBytes],
    ["memory utilization", "memory_percent", formatResourcePercent],
    ["PIDs", "pids", formatPIDAverage],
  ];
  for (const item of stores) {
    for (const [label, key, formatter] of gauges) {
      const gauge = item.resources ? item.resources[key] : null;
      const p95Formatter = key === "pids" ? formatPIDCount : formatter;
      const peakFormatter = key === "pids" ? formatPIDCount : formatter;
      lines.push(`| ${item.store} | ${label} | ${formatter(gauge && gauge.avg)} | ${p95Formatter(gauge && gauge.p95)} | ${peakFormatter(gauge && gauge.peak)} |`);
    }
  }

  lines.push("");
  lines.push("### Cumulative Deltas and Status");
  lines.push("");
  lines.push("| metadata store | network received | network sent | block read | block written | restarts | OOM observed | OOM final |");
  lines.push("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |");
  for (const item of stores) {
    const resources = item.resources;
    lines.push([
      item.store,
      formatBytes(counterDelta(resources, "network_io", "rx_bytes")),
      formatBytes(counterDelta(resources, "network_io", "tx_bytes")),
      formatBytes(counterDelta(resources, "block_io", "read_bytes")),
      formatBytes(counterDelta(resources, "block_io", "write_bytes")),
      resources ? formatNumber(resources.restart_count.delta, 0) : "n/a",
      resources ? formatBoolean(resources.oom_killed.observed) : "n/a",
      resources ? formatBoolean(resources.oom_killed.final) : "n/a",
    ].join(" | ").replace(/^/, "| ").replace(/$/, " |"));
  }
}

function counterDelta(resources, group, counter) {
  if (!resources || !resources[group] || !resources[group][counter]) {
    return null;
  }
  return resources[group][counter].delta;
}
...
function formatPIDAverage(value) {
  return formatNumber(value, 2);
}

function formatPIDCount(value) {
