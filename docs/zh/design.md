...
后台刷新路径包括定时 `prefetch` / `manifest_refresh` 任务、recent-pull refresh debounce，以及 Admin 手动刷新。缓存命中和 stale 命中会发布 `artifact.pulled`；scheduler 把刷新意图写入元数据存储，按制品用 `scheduler.refresh.window` 去重（默认 10 分钟），到期后通过 runtime 的 `Refresher` capability 消费。后台刷新路径通过显式 refresh 请求绕过本地优先规则，主动访问上游并在内容变化时更新本地元数据和对象缓存。
