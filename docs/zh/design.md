...
container registry、Go modules、npm、PyPI、Maven 和 dist 二进制分发镜像都是一等依赖生态。配置按生态拆分为 `container`、`go`、`npm`、`pypi`、`maven` 和 `dist` 块，各生态在 `internal/ecosystems/*` 下维护自己的 endpoint service 和 runtime 实现。

container 生态对外暴露兼容 Registry 的 pull API，并通过 container alias 将请求路由到不同上游 registry。Go、npm、PyPI、Maven 和 dist 生态分别在自己的路径前缀下暴露 read-through proxy cache API，并将请求路由到对应生态配置块里的上游。
...
flowchart LR
  ...
  container --> cachebackend["KV 缓存后端<br/>memory / Redis / Valkey"]
  golang --> cachebackend
  npm --> cachebackend
  pypi --> cachebackend
  maven --> cachebackend
...
Go alias 只在 `go` 配置块内解析，不与 container、npm、PyPI、Maven 或 dist alias 共享命名空间。
...
npm、PyPI、Maven 和 dist 都是一等依赖代理生态，并使用各自独立的 alias 命名空间：
...
当前 capability 覆盖有意保持不对称。container runtime 支持预测性 `prefetch`，因为 OCI 拉取已经依赖 mirror 打分以及 manifest/blob 预热。Go、npm、PyPI、Maven 和 dist 支持通用 endpoint `probe` capability，也通过同一 runtime 注册边界支持 recent-pull `prefetch` rewarm；后续增加各生态自己的版本预测时不需要改调度器装配。

Go、npm、PyPI、Maven 和 dist 的 runtime 仍由各生态包拥有协议细节，但共用轻量的 `internal/depruntime` glue：upstream 映射、runtime 声明 job、手动刷新 job wrapper、prefetch/refresh 响应 drain 等重复外围流程集中在这里。路由解析、cache key、上游请求、内容改写和刷新模式保持在各生态 service 内部，避免把协议差异塞进调度器或共享包。
...
- 配置 Redis 或 Valkey 时使用分布式缓存填充 lease，协调跨副本的制品和 container blob 填充
...
SQL repository 对简单 key 更新优先使用 `dbx/repository` 的 typed patch API（`PatchSet`、`KeySet`、`Part`），把重复的 “by key update + error wrap” 收敛到共享 helper。非主键唯一 key 的 upsert 不作为默认路径，因为它会扩大跨数据库差异并可能覆盖不该更新的内部 id/timestamp 字段。
...
container runtime 对同 key 的上游 token、manifest、blob store 和 tag index 构建使用 typed `singleflightx` 合并并发请求，减少重复上游访问和运行时类型断言。去重 key 仍由具体协议路径负责生成，例如 manifest key 保留 `Accept` 维度，blob 返回前仍检查 repository-to-blob 关系。

Go、npm、PyPI、Maven 和 dist 的不可变响应缓存共用 `internal/artifactcache.Store`。这个组件把 metadata、object store 和 `FillTracker` 收敛成一个可注入的缓存服务；各生态仍保留自己的路由、cache key、上游请求和内容改写逻辑。`FillTracker` 在单进程内使用 `collectionx` 并发 map 合并同 key 的填充，避免同一副本内重复回源。

当 `cache.backend` 配置为 Redis 或 Valkey 时，`internal/cache/backend.LeaseBackend` 会提供分布式 lease。第一个拿到 lease 的副本负责回源并提交 object/metadata；其他副本等待共享 object store 中出现已提交对象后再返回缓存命中。lease 只负责协调并发填充，不替代 SQL metadata 或 object store。多副本部署仍需要共享 MySQL/PostgreSQL metadata 和共享 S3/SFTP object store。

container blob streaming 使用同一类分布式 lease，但执行模型不同：owner 副本会在持有 blob fill lease 时边回源、边返回客户端、边写入 object store；其他副本等待 owner 提交 blob metadata 后从共享对象存储读取。未命中的 range 请求也会等待完整 blob 填充完成，再从本地对象切片返回。若 Redis/Valkey 不可用，服务会保留本地并发合并能力并继续可用，但不同副本之间可能重复回源。
...
- logger、config、auth、scheduler、worker、admin、store、cache backend 和 artifact cache 是共享 module；container 拥有的 upstream、registry tooling、suggestion 和 Docker daemon integration 保持在 container 生态模块集合中
