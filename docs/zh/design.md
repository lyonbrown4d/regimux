...
当前 capability 覆盖有意保持不对称。container runtime 先支持 `probe` 和 `prefetch`，因为 OCI 拉取已经依赖 mirror 打分以及 manifest/blob 预热。Go、npm、PyPI 和 Maven 通过相同 runtime 注册边界接入 fetch/cache 行为，后续增加调度能力时不需要改调度器装配。
...
预拉取会基于拉取历史预测可能的后续 tag，然后通过正常缓存路径预热 manifest 和 blob。调度器通过 runtime 的 `prefetch` capability 调用它，因此其他生态也可以在同一种任务形态后面实现自己的预测和预热逻辑。运行记录和结果会存入元数据，并展示在 Admin UI。
