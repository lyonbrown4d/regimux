...
Go、npm、PyPI 和 Maven 的 runtime 仍由各生态包拥有协议细节，但共用轻量的 `internal/depruntime` glue：upstream 映射、runtime 声明 job、手动刷新 job wrapper、prefetch/refresh 响应 drain 等重复外围流程集中在这里。路由解析、cache key、上游请求、内容改写和刷新模式保持在各生态 service 内部，避免把协议差异塞进调度器或共享包。
