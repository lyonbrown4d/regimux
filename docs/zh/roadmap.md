...
- 增加 Go module proxy read-through cache，支持根路径 GOPROXY 和兼容路径 `/go/{alias}/{module}/@v/...`。已完成。
- Go proxy 优先 upstream alias 为 `golang`，上游为 `https://proxy.golang.org`。可通过 `GOPROXY=http://localhost:5000` 使用。已完成。
