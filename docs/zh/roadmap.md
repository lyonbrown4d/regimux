...
- 保持 OCI / Docker Registry V2 兼容路径 `/v2/{alias}/...`，作为现有能力继续维护。
- 增加 upstream 生态类型：`oci`、`go`、`maven`、`pypi`、`npm`。Go 已完成；Maven、PyPI 和 npm 预留配置枚举。
- 增加 Go module proxy read-through cache，路径为 `/go/{alias}/{module}/@v/...` 和 `/go/{alias}/{module}/@latest`。已完成。
- Go proxy 默认 upstream alias 为 `golang`，上游为 `https://proxy.golang.org`。可通过 `GOPROXY=http://localhost:5000/go/golang` 使用。已完成。
- 将 Go proxy 响应按内容 sha256 写入对象存储，并通过元数据记录请求路径到对象 digest 的映射。已完成。
- Maven 下一步实现 read-through 仓库布局缓存，优先支持 release 制品、`maven-metadata.xml` 和 checksum 文件。
- PyPI 下一步实现 PEP 503 simple index 缓存、包名 normalize 和文件链接重写。
- npm 最后实现，重点处理 packument、dist-tags、scoped package、tarball URL rewrite 和 integrity。
