...
RegiMux 会从只读 OCI / Docker Registry V2 代理镜像逐步升级为研发依赖缓存网关。短期仍保持只读和代理缓存定位，先覆盖 OCI、Go、Maven、PyPI，npm 放到最后。

## 近期

### 研发依赖缓存网关

- 保持 OCI / Docker Registry V2 API `/v2/{containerAlias}/...`，作为稳定 container 路径。
- 使用独立顶层生态配置块：`container`、`go`、`npm`、`pypi` 和 `maven`。
- 将 registry、mirror、probe 和 prefetch 行为通过 `dix` 注册的生态 runtime 承载；调度器按 runtime capability 分发，而不是直接导入具体生态实现。
- container 先作为具备预测性定时 `prefetch` 的 runtime；Go、npm、PyPI 和 Maven 已通过同一 runtime 抽象共享定时 endpoint `probe` 和 recent-pull prefetch rewarm。
...
- npm 使用 `/npm/{npmAlias}/...`，重点处理 packument、dist-tags、scoped package、tarball URL rewrite 和 integrity。
- PyPI 使用 `/pypi/{pypiAlias}/...`，实现 PEP 503 simple index 缓存、包名 normalize 和文件链接重写。
- Maven 使用 `/maven/{mavenAlias}/...`，实现 read-through 仓库布局缓存，优先支持 release 制品、`maven-metadata.xml` 和 checksum 文件。
