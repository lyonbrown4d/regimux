...
RegiMux 已经从只读 OCI / Docker Registry V2 代理镜像扩展为研发依赖缓存网关。产品继续保持只读和代理缓存定位，container registry、Go、npm、PyPI 和 Maven 都作为一等生态。
...
- 继续用真实包管理器客户端和各生态边界场景强化 npm、PyPI 和 Maven 兼容性。
