...
- [configs/split](../../configs/split)：按目录加载拆分后的 HCL 示例。

`--config` 可以指向单个 `.hcl` 文件，也可以指向一个目录。指向目录时，RegiMux 会按文件名字典序加载该目录当前层级下的所有 `.hcl` 文件。这样小部署仍可使用单文件，大部署可以按职责拆分配置，例如：
...
靠后的文件可以覆盖靠前文件中的同名配置，因此建议在需要顺序时使用数字前缀。子目录会被忽略。

[configs/split](../../configs/split) 里的拆分示例采用这套结构：

- [00-server.hcl](../../configs/split/00-server.hcl)：监听地址、认证、日志和元数据缓存默认值。
- [10-store.hcl](../../configs/split/10-store.hcl)：metadata 数据库和 object store。
- [20-container.hcl](../../configs/split/20-container.hcl)：容器 registry 依赖代理 alias。
- [30-languages.hcl](../../configs/split/30-languages.hcl)：Go、npm、PyPI、Maven 代理 alias。
- [40-dist.hcl](../../configs/split/40-dist.hcl)：Gradle、Electron、Playwright、Node.js 等通用二进制分发 mirror。
- [90-policy.hcl](../../configs/split/90-policy.hcl)：依赖 allow/block 规则。
