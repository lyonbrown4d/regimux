...
func TestServiceRewritesMetadataTarballsAndCaches(t *testing.T) {
	...
	first, err := service.Get(ctx, npmproxy.Request{
		Alias:        "npmjs",
		Tail:         "left-pad",
	...
	second, err := service.Get(ctx, npmproxy.Request{
		Alias:        "npmjs",
		Tail:         "left-pad",
		ProxyBaseURL: "https://cache.example.test",
	})
...
func TestServiceHandlesScopedMetadataPaths(t *testing.T) {
	for _, tail := range []string{"@scope/pkg", "@scope%2fpkg"} {
		t.Run(tail, func(t *testing.T) {
			...
			service := newTestService(ctx, t, upstream.URL, 5*time.Minute)
			resp, err := service.Get(ctx, npmproxy.Request{
				Alias: "npmjs",
				Tail:  tail,
			})
...
func TestServiceRefreshesExpiredMetadata(t *testing.T) {
	...
	service := newTestService(ctx, t, upstream.URL, time.Nanosecond)
	first, err := service.Get(ctx, npmproxy.Request{Alias: "npmjs", Tail: "left-pad"})
	requireNoError(t, "first metadata get", err)
	_ = readJSON(t, first)
	time.Sleep(time.Millisecond)
	second, err := service.Get(ctx, npmproxy.Request{Alias: "npmjs", Tail: "left-pad"})
	requireNoError(t, "second metadata get", err)
	_ = readJSON(t, second)
	if requests != 2 {
...
func newTestService(ctx context.Context, t *testing.T, upstreamURL string, metadataTTL time.Duration) *npmproxy.Service {
	t.Helper()
	...
	return npmproxy.NewService(npmproxy.ServiceDependencies{
...
func readJSON(t *testing.T, resp *npmproxy.Response) map[string]any {
	t.Helper()
	body := readBody(t, resp)
...
func readBody(t *testing.T, resp *npmproxy.Response) string {
	t.Helper()
