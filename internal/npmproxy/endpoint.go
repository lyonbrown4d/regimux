...
func (e *Endpoint) Register(registrar httpx.Registrar) {
	...
	httpx.MustGroupGet(group, "npm/{alias}/{tail...}", e.get)
	httpx.MustGroupRoute(group, http.MethodHead, "npm/{alias}/{tail...}", e.head)
}
...
func (e *Endpoint) dispatch(ctx context.Context, in *input, method string) (*output, error) {
	if in == nil {
		return plainError(http.StatusBadRequest, "npm proxy input is required"), nil
	}
...
func outputFromResponse(method string, resp *Response) *output {
	if resp == nil {
		return plainError(http.StatusBadGateway, "npm proxy response is empty")
	}
	out := &output{
		Status:        resp.Status,
	...
	out.Body = streamWithStatus(resp.Status, httpx.StreamWriter(func(writer io.Writer) {
		defer closeReadCloser(resp.Body, nil, "close npm proxy response body")
