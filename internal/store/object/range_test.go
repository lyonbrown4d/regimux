...
func TestParseRange(t *testing.T) {
	...
	tests := []struct {
		header string
		want   object.HTTPRange
	}{
		{"bytes=0-99", object.HTTPRange{Start: 0, End: 99}},
		{"bytes=500-", object.HTTPRange{Start: 500, End: -1}},
		{"bytes=-250", object.HTTPRange{Start: -1, End: 250}},
	}
	for _, tt := range tests {
		got, err := object.ParseRange(tt.header)
		if err != nil {
...
func TestParseRangeRejectsInvalid(t *testing.T) {
	...
	for _, header := range []string{"items=0-1", "bytes=0-1,2-3", "bytes=5-4", "bytes=-0"} {
		if _, err := object.ParseRange(header); err == nil {
...
func TestRangeResolve(t *testing.T) {
	...
	got, err := (object.HTTPRange{Start: 500, End: -1}).Resolve(1000)
	if err != nil {
		t.Fatalf("Resolve open ended error = %v", err)
	}
	if *got != (object.HTTPRange{Start: 500, End: 999}) {
	...
	got, err = (object.HTTPRange{Start: -1, End: 250}).Resolve(1000)
	if err != nil {
		t.Fatalf("Resolve suffix error = %v", err)
	}
	if *got != (object.HTTPRange{Start: 750, End: 999}) {
...
func TestRangeString(t *testing.T) {
	...
	tests := []struct {
		...
		{object.HTTPRange{Start: 0, End: 99}, "bytes=0-99"},
		{object.HTTPRange{Start: 500, End: -1}, "bytes=500-"},
		{object.HTTPRange{Start: -1, End: 250}, "bytes=-250"},
	}
	for _, tt := range tests {
