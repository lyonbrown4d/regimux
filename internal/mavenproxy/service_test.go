...
func TestServiceCachesReleaseArtifactByPath(t *testing.T) {
	...
	first, err := service.Get(ctx, mavenproxy.Request{
		Alias: "central",
	...
	second, err := service.Get(ctx, mavenproxy.Request{
		Alias: "central",
		Tail:  "com/acme/demo/1.2.3/demo-1.2.3.jar",
	})
...
func TestServiceStoresShortTTLForMetadataAndSnapshots(t *testing.T) {
	...
	metadataResp, err := service.Get(ctx, mavenproxy.Request{
		Alias: "central",
	...
	snapshotResp, err := service.Get(ctx, mavenproxy.Request{
		Alias: "central",
...
func TestServiceUsesMirrorsWhenRegistryIsEmpty(t *testing.T) {
	...
	resp, err := service.Get(ctx, mavenproxy.Request{
		Alias: "mirror",
...
func TestServiceRejectsNonMavenUpstream(t *testing.T) {
	ctx := context.Background()
	service := mavenproxy.NewService(mavenproxy.ServiceDependencies{
	...
	_, err := service.Get(ctx, mavenproxy.Request{
		Alias: "default",
		Tail:  "com/acme/demo/1.2.3/demo-1.2.3.jar",
	})
	if err == nil {
...
func newTestService(ctx context.Context, t *testing.T, upstreams map[string]config.DependencyUpstreamConfig) (*mavenproxy.Service, meta.Store) {
	t.Helper()
	...
	return mavenproxy.NewService(mavenproxy.ServiceDependencies{
		Config: config.Config{
			Maven: upstreams,
		},
		Metadata: db,
...
func assertBody(t *testing.T, resp *mavenproxy.Response, want string) {
	t.Helper()
