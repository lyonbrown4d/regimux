...
func addSchedulerRows(rows *collectionlist.List[ConfigRow], cfg config.SchedulerConfig) {
	...
	addRow(rows, "scheduler.refresh.enabled", boolString(cfg.Refresh.Enabled))
	addRow(rows, "scheduler.refresh.window", durationString(cfg.Refresh.Window))
	addRow(rows, "scheduler.refresh.distributed", boolString(cfg.Refresh.Distributed))
...
func addManifestRefreshEcosystemRows(rows *collectionlist.List[ConfigRow], cfg config.SchedulerManifestRefreshConfig) {
	...
	ecosystemNames := collectionlist.NewList(collectionmapping.NewMapFrom(cfg.Ecosystems).Keys()...).Sort(strings.Compare)
	ecosystemNames.Range(func(_ int, ecosystemName string) bool {
